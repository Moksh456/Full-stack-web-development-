...
j.s is the most demanding skill for the we devloper in the market mainly we focous on the writing a neat and clean code for the company or client.
...
EMPTY STRING  OR 0 VALUE IS  =>  EQUAL TO A FALSY VALUE;
...
comparison operators -> 
 ...
 === check the value and also check the datatype both.
...
for select id in js file  -> docment.queryselector(#id_name);
...
async suits in situations -> where js file is intereacting with Dom
...
console.log("Box 1 clicked"); 
   ...
   for these html elements onclick callbackj function should be run (only for when callback functoin is registered on
   them via addEventListener ).
  ...
  first  flow was named as -> event BUBLE
  second flow was named as -> event CAPTURE
...
local storage and session storage
      ...
      local storage => local storage means jo data broswer bamd hone ke baad bhi delete nhi hoga.

      session storage =>  ye apna data temporaryly store krta h means if the tab close and data will dissapear

      cookies => ye bhi data store krta hai or aapka data borswer ki cookies naam ki propeerty  me save hota h . this cookie is made
       for the less data or light data like pass name etc..



       local storage =>   1. how to save data.   =>   .setItem(value name, value)
                          2. how to fetch data.  =>   .getItem(value name)
                          3. how to remove data. =>   .removeItem(value name )
                          4. how to update.      =>   .setItem(value name, value)
                          5. to clear all thing  =>   .clear()


            1. save the data =>  rage.setItem("this is for to declare the name of value", "this is for the value");
                if we save the value through this so our value is the permanent in the brouswer localStorage.
            
            2. how to fetch =>  localStorage.getItem("name the value whic you want to fetch"); 

            3. 3. how to remove data. => localStorage.removeItem(value name )   this is used for the removing value

            4. how to update => localStorage.setItem("name of the value", value)  if the value is not declare it will make new value 
            and if the value will be the daclared in the past so it will override the older value and make new value at the place of 
            oldest value. and update the value.

            local storage me array object jaise cheej sotre nhi kr skte isliye unhe string bna ke store krte h JSON.
            Stringfy() wale method se or baad me use krte time JSOM.pasre() wale method se nikalte h use krne ke liye.


session storage  => this is the temporary memomry of the broswer if the we close borswer then the all session storage item will be the disappear.
        all the functality is same like local storag set, save, fetch, remove, update etc....


cookies =>  borswe me chota data store krne ke liye use hota h isme 4kb ke lagbhag data store kr skti h or localStorage 5mb tak 
            cookies me jo bhi data sotre hoga wo page reload hote hi SERVER pe chla jayega.
...
Rest Operator(...)  :
  ...
  rest operator also works with objects. 
    ...
    usecase :
      design the function that can accept unlimmited arguments as inputs.
...
Async Javascript :
  ...
  sync and async
   sync = jab code line by line clta h use syncroion js bolte h 
   async = aesa code jo chle ke liye rewady ho jaye or baad me chle use async code khte h 
...
generators :
  ...
  next() method returns object having 2 properties :
    ...
    done -> boolean value telling wheather exeution of generator function is 
    completed or not.
...
Iso8601 {for standardziation}
epoch {backend syntems generally work with epoch}

_______________________________________________________________________

 PRoperty Descriptors : 
  Every property in any object in js can have its own configrution :
  ...
  name and age can have its own custom configrations :

  1. writabble -> true/false(despcts whether any value can be overridden or not)

  2. enumrable -> (depicts whether this property should be present while looping or not) object.keys

  3. configurable -> whether above canfigruration like writable, enumrable can be change in future or not.
...
End of javascript Good by Enjoy react party by the github this repo